...
func main() {
	...
	transport, err := sf.NewSnowflakeTransport(*iceServersCommas, *brokerURL, *frontDomain, *keepLocalAddresses || *oldKeepLocalAddresses, *max)
	if err != nil {
		log.Fatal("Failed to start snowflake transport: %s", err.Error())
	}
