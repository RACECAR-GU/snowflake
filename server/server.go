...
func acceptLoop(ln net.Listener) {
	...
	for {
		...
		log.Printf("Snowflake connection accepted")

		addr := conn.RemoteAddr().String()
		if addr == (sf.ClientAddress{}).String() {
			addr = ""
		}
		statsChannel <- addr != ""
...
func main() {
	...
	for _, bindaddr := range ptInfo.Bindaddrs {
		...
		if disableTLS {
			args.Add("tls", "no")
			transport = sf.NewSnowflakeTransportServer(nil)
		} else {
			args.Add("tls", "yes")
			for _, hostname := range acmeHostnames {
				args.Add("hostname", hostname)
			}
			transport = sf.NewSnowflakeTransportServer(certManager.GetCertificate)
		}
		ln, err := transport.Listen(bindaddr.Addr.String())
		if err != nil {
			log.Fatal("Error listening for Snowflake connecitons: %s", err.Error())
		}
		go acceptLoop(ln)
		if err != nil {
			log.Printf("error opening listener: %s", err)
